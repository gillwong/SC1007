...
void levelOrderTraversal(BSTNode* root)
{
	...
	while(!isEmpty(temp.head)) {
		BSTNode *curr = dequeue(&(temp.head), &(temp.tail));
		printf("%d ", curr->item);
		if(curr->left != NULL) enqueue(&(temp.head), &(temp.tail), curr->left);
		if(curr->right != NULL) enqueue(&(temp.head), &(temp.tail), curr->right);
	}
}
