...
int BFS (Graph g, int v, int w){
	...
	if (v < 1 || v > g.V || w < 1 || w > g.V) return -1;
    
    Queue q;
    q.size = 0;
    q.head = q.tail = NULL;

    char *visited = malloc(sizeof(char) * g.V);
    ...
    while (!isEmptyQueue(q)) {
        int frontVertex = getFront(q);
        dequeue(&q);
        if (frontVertex == w) {
            free(visited);
            return distance[frontVertex - 1];
        }
        ListNode *ptr = g.list[frontVertex - 1];
        while (ptr) {
            if (visited[ptr->vertex - 1] == 0) {
                enqueue(&q, ptr->vertex);
                visited[ptr->vertex - 1] = 1;
				distance[ptr->vertex - 1] = distance[frontVertex - 1] + 1;
            }
            ptr = ptr->next;
        }
    }
