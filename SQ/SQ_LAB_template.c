...
int main()
{
	...
	if (palindrome(word1) == 0) printf("word1 is a palindrome\n");  // +
	else printf("word1 is not a palindrome\n");  // +
	if (palindrome(word2) == 0) printf("word2 is a palindrome\n");  // +
	else printf("word2 is not a palindrome\n");  // +
...
int palindrome(char *word){
	...
	while(word[ii] != '\0') {
		if(word[ii] != ' ') {
			enqueue(&q, toupper(word[ii]));
			push(&s, toupper(word[ii]));
		}
		++ii;
	}
	while(!isEmptyQueue(&q)) {
		if(dequeue(&q) != pop(&s)) return -1;
	}
	return 0;
}
